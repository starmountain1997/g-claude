...
1. **Model on disk** — use `/model-download`. Never pass an online model ID to vLLM; always use a local path.

## Task Specifics

- **Installation**: [vllm-install.md](vllm-install.md) — version-pinning between `vllm` and `vllm-ascend`, editable install
- **Running & Tuning**: [vllm-run.md](vllm-run.md) — scenario interview → offline validation → graph mode → online serving
